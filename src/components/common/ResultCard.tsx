...
function InfoRow({ label, value, valueColor, labelColor, dividerAfter }: InfoRowData) {
  return (
    <>
      <div className="flex items-start justify-between gap-4">
        <span className={`shrink-0 text-[14px] ${labelColor ?? "text-[#68677E]"}`}>{label}</span>
        <span className={`text-right text-[14px] font-semibold ${valueColor ?? "text-[#23222A]"}`}>
          {value}
        </span>
      </div>
