...
const getSeatList = async (
  showScheduleId: number,
  sessionToken: string = "60501eb8-89ce-4e31-95d4-c48ca3fc0b18" // swagger로 받은 session token 하드코딩
  ...
  const { data } = await api.get<ApiResponse<{ sections: ApiSection[] }>>(
    ...
    { 
      headers: {
        ...
        // access token 하드 코딩
        "Authorization": `Bearer eyJhbGciOiJIUzI1NiJ9..QgyCdFZI2W06tqn84BLF84tT3R-5aNT2e2PwDgmt9iM` // 아까 받은 토큰
      }
    }
  );
...
const holdSeats = async (
  ...
  seatIds: number[],
  // session token 자리
  sessionToken: string = "60501eb8-89ce-4e31-95d4-c48ca3fc0b18",
): Promise<string | null> => { 
  const { data } = await api.post<ApiResponse<string>>(
    ...
    { 
      headers: {
        ...sessionHeader(sessionToken),
        // access token 자리
        "Authorization": `Bearer eyJhbGciOiJIUzI1NiJ9..QgyCdFZI2W06tqn84BLF84tT3R-5aNT2e2PwDgmt9iM`,
      }
    }
  );
  return data.data;
