...
const getSeatList = async (
  showScheduleId: number,
  sessionToken: string = "282396ba-eb09-487e-a54c-f4cb6c98a487" // swagger로 받은 session token 하드코딩
  ...
  const { data } = await api.get<ApiResponse<{ sections: ApiSection[] }>>(
    ...
    { 
      headers: {
        ...
        // access token 하드 코딩
        "Authorization": `Bearer eyJhbGciOiJIUzI1NiJ9..J1j1RNLW-P3wmcAe7oZbd3gkkMD4NDM7QTnewWVPc-E` // 아까 받은 토큰
      }
    }
  );
...
const holdSeats = async (
  ...
  seatIds: number[],
  // session token 자리
  sessionToken: string = "282396ba-eb09-487e-a54c-f4cb6c98a487",
): Promise<string | null> => { 
  const { data } = await api.post<ApiResponse<string>>(
    ...
    { 
      headers: {
        ...sessionHeader(sessionToken),
        // access token 자리
        "Authorization": `Bearer eyJhbGciOiJIUzI1NiJ9..J1j1RNLW-P3wmcAe7oZbd3gkkMD4NDM7QTnewWVPc-E`,
      }
    }
  );
  return data.data;
