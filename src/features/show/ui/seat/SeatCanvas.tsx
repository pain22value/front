...
> = {
  OP: { label: "OP", align: "center", getStyle: () => ({ fill: 0xdecffb, stroke: 0x783cf1 }) },
  "1F-A": {
    label: "A",
    align: "right",
  ...
  "1F-B": {
    ...
    align: "center",
    getStyle: (rowName) =>
      Number(rowName) <= 15 ? { fill: 0xdecffb, stroke: 0x783cf1 } : { fill: 0xc2f2d8, stroke: 0x25b07d },
  },
  "1F-C": {
    label: "C",
    align: "left",
...
export const SeatCanvas = ({ sections, selectedSeats, onSeatClick }: SeatCanvasProps) => {
  ...
  useEffect(() => {
    ...
    const LOGICAL_H =
      STAGE_H + OP_H + GAP_OP_1F + SEC1_H + CONSOLE_H + BOX1_PAD + BADGE_H + SEC2_H + BOX2_PAD + BADGE2_H + 20;
      ...
      .then(() => {
        ...
        opSecRows.forEach((row) => {
          ...
          row.seats.forEach((seat, ci) => {
            ...
            const style =
              STATUS_OVERRIDE[seat.status] !== undefined
        ...
        const renderSection = (sectionId: string, baseX: number, align: Align, maxCols: number, startY: number) => {
          ...
          sec.rows.forEach((row) => {
            ...
            if (align === "left") {
              app.stage.addChild(
                makeText(row.rowName, { fontSize: 7, fill: 0x999999 }, rx + maxCol * STEP + 3, ry + 1),
              );
            } else {
              const rn = makeText(row.rowName, { fontSize: 7, fill: 0x999999 }, rx - 4, ry + 1);
              rn.anchor.set(1, 0);
        ...
        ["1F-A", "1F-B", "1F-C"].forEach((sid) => {
          const lbl = makeText(
            SECTION_CONFIG[sid].label,
            { fontSize: 10, fill: 0x555555, fontWeight: "bold" },
            0,
            SEC1_Y - 16,
          );
        ...
        const a2MaxCols = Math.max(
          ...(sections.find((s) => s.sectionId === "2F-A")?.rows.flatMap((r) => r.seats.map((s) => s.col)) ?? [5]),
        );
